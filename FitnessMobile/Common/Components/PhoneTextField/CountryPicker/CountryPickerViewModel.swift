...
class CountryPickerViewModel: BaseViewModel {
    ...
    @Published var filteredCountries: [Phone] = []
    private var countries: [Phone] = [] {
        didSet {
            filterCountries()
        }
    }
    ...
    func filterCountries() {
        if !searchText.isEmpty {
            ...
            filteredCountries = countries.filter({$0.countryName.lowercased().contains(lowercasedSearchText)})
        } else {
    ...
    func mapCountries() {
        ...
        for countryCode in countryCodes {
            let newCountry = Phone(countryName: countryName(countryCode), number: "", phoneCode: countryPhoneCodes[countryCode] ?? "", countryCode: countryCode)
            result.append(newCountry)
        }
    ...
    let countryPhoneCodes  = ["AF":"93","AL":"355","DZ":"213","US":"1",
